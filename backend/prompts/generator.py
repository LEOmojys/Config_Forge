...
8. 掉落概率 chance 是概率不是权重：同一掉落组内所有 chance 的总和必须 ≤ 1.0。
   如有多个掉落项，请按比例归一化，使总和不超过 1.0（保留两位小数）
9. 任务目标 target_id 的类型约束：kill 目标的 target_id 必须是【可用资源】中已有的
   怪物模板 ID（tpl_ 前缀），不得使用怪物实例 ID（mon_ 前缀）；collect 目标的
   target_id 必须是已有的道具 ID（it_ 前缀）
